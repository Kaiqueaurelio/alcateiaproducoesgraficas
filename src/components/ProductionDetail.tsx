...
export function ProductionDetail({
  ...
  async function saveAssignee() {
    ...
    const { error } = await db
      .from("orders")
      .update({ assignee_name: assignee || null })
      .eq("id", orderId);
  ...
  async function addUpdate() {
    ...
    const { error } = await db.from("production_updates").insert({
      order_id: orderId,
      author_id: user?.id ?? null,
      author_name: authorName,
      status: statusChanged ? (newStatus as any) : null,
      note: note.trim(),
    });
    if (error) {
      setSaving(false);
  ...
  async function delUpdate(id: string) {
    ...
    const { error } = await db.from("production_updates").delete().eq("id", id);
    if (error) return toast.error(error.message);
    load();
  }
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
            <div>
              ...
              <div className="font-medium">{fmtDate(order.deadline)}</div>
            </div>
            <div>
            ...
            <div>
              ...
              <div className="font-mono font-medium">{brl(order.total)}</div>
            </div>
            <div>
              <div className="text-xs text-muted-foreground">Pago</div>
              <div className="font-mono font-medium">{brl(order.paid)}</div>
            </div>
          </div>
          ...
          <div className="flex items-end gap-2">
            ...
            <Button variant="outline" onClick={saveAssignee}>
          ...
          <div className="border rounded-lg p-3 bg-card space-y-2">
            ...
            <Textarea
              rows={2}
              placeholder="Ex.: Iniciei a arte do Instagram, devo terminar em 2h"
              value={note}
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <div className="flex-1">
                <Select value={newStatus} onValueChange={setNewStatus}>
                  <SelectTrigger>
                    <SelectValue placeholder="Mudar status (opcional)" />
                  </SelectTrigger>
                  <SelectContent>
                    {PROD_STATUSES.map((s) => (
            ...
            <div className="space-y-2">
                ...
                <div key={u.id} className="border-l-2 border-gold pl-3 py-1 group">
                  ...
                  {u.status && (
                    <Badge variant="outline" className="text-[10px] mt-1">
                      → {PRODUCTION_STATUS_LABEL[u.status]}
                    </Badge>
                  )}
              ...
              {!updates.length && (
                <div className="text-sm text-muted-foreground italic">
                  Sem atualizações ainda. Adicione a primeira acima.
                </div>
              )}
            </div>
          </div>
